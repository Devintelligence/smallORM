<?php
/* Extended by Ralph Klepper 2013 siehe End of file */
/**
...
class nomad_mimemail
{
  /**
	...
	function _build_header($content_type)
	{
		...
		$this->mail_header .= "Message-Id: <" . md5(uniqid(rand())) . ".RK_mimemail@" . $_SERVER['LOCAL_ADDR'] . ">" . BR;
	...
	function _build_body()
	{
		switch ($this->_parse_elements()){
			...
			case 7:  // Plain Text + HTML + Attachments
				...
				$this->mail_body .= "Content-Transfer-Encoding: 7bit" . BR . BR;
				$this->mail_body .= $this->mail_text . BR . BR;
				$this->mail_body .= "--" . $this->boundary_alt . BR; // Ralph Klepper Fehler bereinigt -> Doppeltes BR entfernt
				$this->mail_body .= "Content-Type: text/html; charset=\"$this->charset\"" . BR;
				$this->mail_body .= "Content-Transfer-Encoding: 7bit" . BR. BR;
				$this->mail_body .= $this->mail_html . BR; // Ralph Klepper Fehler bereinigt -> Doppeltes BR entfernt
	...
	function _parse_elements()
	{
		...
		if (!empty($this->mail_html)){
			...
			if (empty($this->mail_text)){
				$this->mail_text = strip_tags(str_ireplace("<br>", BR, $this->mail_html));
				$this->mail_type = $this->mail_type + 1; // Plain Text
			}
		}
	...
	{
		$return = is_email_valid($mail);
		// Anpassung Ralph Klepper wegenen PHP5.4
...
/*----------------- Netzwerk funktionen --------------------- */
/*----------------- Erweiterung Ralph Klepper für PHP5.4 --------------------- */
	...
	if (!function_exists('is_email_valid')) {
		function is_email_valid($email) {
			...
			Thanks to Douglas Lovell isoftware engineer with IBM Research
			*/
			 $isValid = true;
